...
public class EmployeeDaoImpl implements EmployeeDao {
    ...
    public List<Employee> selectAllEmployee() throws SQLException {
        String sql = "select * from employee e inner join department d on e.pid=d.pid";
    ...
    @Override
    public List<Employee> selectConditionEmployee(Integer pidEmp) throws SQLException {
        String sql = "select * from employee e inner join department d on e.pid=d.pid where e.pid=?";
        Connection conn = JdbcUtils.getConnection();
        PreparedStatement prstmt = conn.prepareStatement(sql);
        prstmt.setObject(1, pidEmp);
        ResultSet resultSet = prstmt.executeQuery();
        List<Employee> employeeList = new ArrayList<>();
        while (resultSet.next()) {
            employeeList.add(new Employee(
