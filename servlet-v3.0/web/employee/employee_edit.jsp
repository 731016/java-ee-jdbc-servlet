...
<div style="width: 1000px;margin: 0 auto;text-align: center">
    ...
    <span style="color: red; display: block;margin: 10px 0">双击头像可修改!!!</span>
</div>
        ...
        <tr>
            ...
            <c:choose>
                <c:when test="${employeeOne.getSalary()>=8000}">
                    <td style="border:2px dashed olivedrab;">
                        <input name="salary" type="number" value="${employeeOne.getSalary()}">
                    </td>
                </c:when>
                <c:when test="${employeeOne.getSalary()>=6000}">
                    <td style="border:2px dashed orange;">
                        <input name="salary" type="number" value="${employeeOne.getSalary()}">
                    </td>
                </c:when>
                <c:otherwise>
                    <td style="border:2px dashed crimson">
                        <input name="salary" type="number" value="${employeeOne.getSalary()}">
                    </td>
                </c:otherwise>
            </c:choose>
